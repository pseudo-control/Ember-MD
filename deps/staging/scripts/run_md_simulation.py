...
def run_production(system: Any, modeller: Any, equilibrated_state: Any, output_dir: str, job_name: str, production_ns: float, platform_name: str, temperature_k: float = 300, seed: int = 0) -> str:
    ...
    while steps_completed < total_steps:
        ...
        # Format: current_ns/total_ns:ns_per_day:eta_seconds
        print(f'PROGRESS:production:{ns_completed:.1f}/{production_ns:.1f}:{ns_per_day:.1f}:{eta_s:.0f}', flush=True)

    print(f'PROGRESS:production:{production_ns:.1f}/{production_ns:.1f}:0:0', flush=True)
