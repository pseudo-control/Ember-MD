...
if __name__ == '__main__':
    ...
    parser.add_argument('--anchor_mode', type=str, choices=['denovo', 'grow'], default='denovo',
                            help='denovo: generate from empty pocket; grow: extend anchor molecule from --sdf_file')
    ...
    frag_base = {
        ...
        'data_base_smiles': np.bytes_(atom_frag_database.smiles)
    }
    ...
    # start to generate
    if args.anchor_mode == 'grow':
        # Grow mode: use anchor molecule as initial state
        from utils.anchor import load_anchor_sdf, anchor_to_initial_state, has_3d_coordinates, prepare_anchor_for_growing

        if args.sdf_file is None:
            raise ValueError("--sdf_file is required for grow mode (anchor molecule)")

        print(f"Loading anchor molecule from: {args.sdf_file}")
        anchor_mol = load_anchor_sdf(args.sdf_file)

        if not has_3d_coordinates(anchor_mol):
            raise ValueError("Anchor SDF must have 3D coordinates for grow mode")

        print(f"Loaded anchor: {Chem.MolToSmiles(anchor_mol)} ({anchor_mol.GetNumAtoms()} atoms)")

        # Remove explicit hydrogens to create attachment points for growing
        anchor_mol = prepare_anchor_for_growing(anchor_mol)
        print(f"Prepared anchor (H removed): {Chem.MolToSmiles(anchor_mol)} ({anchor_mol.GetNumAtoms()} heavy atoms)")

        data_initial_list = anchor_to_initial_state(anchor_mol, pkt_data, transform_ligand)
        print(f"Initialized {len(data_initial_list)} states from anchor molecule")
    else:
        # De novo mode: generate from empty pocket
